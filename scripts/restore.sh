...
if [ -n "$RECOVERY_TARGET_TIME" ]; then
    echo -e "${YELLOW}╔══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${YELLOW}║          Point-in-Time Recovery (PITR) Setup           ║${NC}"
    echo -e "${YELLOW}╚══════════════════════════════════════════════════════════╝${NC}"
    echo ""
    echo -e "${YELLOW}⚠ WARNING: PITR requires manual PostgreSQL configuration${NC}"
    echo ""
    echo -e "${BLUE}To complete PITR setup:${NC}"
    echo ""
    echo "1. Stop PostgreSQL:"
    echo "   sudo systemctl stop postgresql"
    echo ""
    echo "2. Create recovery.signal file:"
    echo "   sudo touch /var/lib/postgresql/15/main/recovery.signal"
    echo ""
    echo "3. Add recovery configuration to postgresql.auto.conf:"
    echo "   sudo bash -c 'cat >> /var/lib/postgresql/15/main/postgresql.auto.conf <<EOT"
    echo "restore_command = 'aws s3 cp s3://\${S3_BUCKET}/wal/%f %p'"
    ...
    echo ""
    echo "5. Monitor recovery progress:"
    echo "   sudo tail -f /var/log/postgresql/postgresql-15-main.log"
    echo ""
    echo -e "${YELLOW}Note: WAL archives must be available in S3 or local directory${NC}"
    echo -e "${YELLOW}The database will automatically promote when target time is reached${NC}"
    echo ""
    read -p "Press Enter to continue with basic restore (PITR must be configured manually)..."
fi
